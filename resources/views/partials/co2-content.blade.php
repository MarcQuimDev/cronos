<main class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 sm:px-0 mb-6 animate-fade-in-up">
        <div class="bg-neutral-900 shadow-xl rounded-lg p-6 border border-neutral-800 hover:border-cyan-500/50 transition-all duration-300 hover:shadow-2xl hover:shadow-cyan-500/10">
            ...
            <div class="relative h-80"><canvas id="co2Chart"></canvas></div>
            @elseif(!$selectedDate)
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-neutral-800">
                    <thead class="bg-black/30">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-400 uppercase tracking-wider">CO2 (ppm)</th>
                    ...
                    <tbody class="bg-neutral-900 divide-y divide-neutral-800">
                        ...
                        @empty
                        <tr><td colspan="3" class="px-6 py-8 text-center text-sm text-neutral-500">No hi ha dades de CO2 disponibles</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    @if($co2Data->hasPages())
    <div class="px-4 sm:px-0 mt-6"><div class="bg-neutral-900 shadow-xl rounded-lg p-4 border border-neutral-800">{{ $co2Data->links() }}</div></div>
    @endif
</main>
...
(function() {
    ...
    const values = chartData.map(item => parseFloat(item.eco2));
    const ctx = document.getElementById('co2Chart');
    if (!ctx) return;
    if (window.co2ChartInstance) { window.co2ChartInstance.destroy(); window.co2ChartInstance = null; }
    window.co2ChartInstance = new Chart(ctx, {
        type: 'line',
        data: { labels, datasets: [{ label: 'CO2 (ppm)', data: values, borderColor: 'rgb(34, 211, 238)', backgroundColor: 'rgba(34, 211, 238, 0.1)', tension: 0.4, fill: true, pointBackgroundColor: 'rgb(34, 211, 238)', pointBorderColor: 'rgb(34, 211, 238)', pointHoverBackgroundColor: 'rgb(255,255,255)', pointHoverBorderColor: 'rgb(34, 211, 238)', pointRadius: values.length > 100 ? 1 : 4, pointHoverRadius: 7, borderWidth: values.length > 100 ? 1.5 : 3 }] },
        options: {
